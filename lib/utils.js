...
export function loadWalletKeypair(keypairFile) {
    const loaded = Keypair.fromSecretKey(new Uint8Array(keypairFile));
    return loaded;
}
export async function generateMultisigMembers(connection) {
    const members = {
    ...
    await Promise.all(Object.values(members).map(async (member) => {
        const sig = await connection.requestAirdrop(member.publicKey, 100 * LAMPORTS_PER_SOL);
        await connection.confirmTransaction(sig);
        console.log(sig);
    }));
    return members;
}
export async function createAutonomousMultisig({ connection, createKey = Keypair.generate(), members, threshold, timeLock, }) {
...
export async function createControlledMultisig({ connection, createKey = Keypair.generate(), configAuthority, members, threshold, timeLock, }) {
    const creator = await generateFundedKeypair(connection);
    const [multisigPda, multisigBump] = multisig.getMultisigPda({
        createKey: createKey.publicKey,
    });
...
export async function generateFundedKeypair(connection) {
    const keypair = Keypair.generate();
    const tx = await connection.requestAirdrop(keypair.publicKey, 1 * LAMPORTS_PER_SOL);
    await connection.confirmTransaction(tx);
    return keypair;
}
export function createTestTransferInstruction(authority, recipient, amount = 1000000) {
...
export function isCloseToNow(unixEpoch, timeWindow = 2000) {
