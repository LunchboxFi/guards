...
import { AddressLookupTableProgram, LAMPORTS_PER_SOL, SystemProgram, TransactionMessage, VersionedTransaction, } from "@solana/web3.js";
import assert from "assert";
import { createAutonomousMultisig, createLocalhostConnection, createTestTransferInstruction, generateFundedKeypair, generateMultisigMembers, range, } from "./utils.js";
...
async () => {
    ...
    for (const member of Object.values(members)) {
        const ix = createTestTransferInstruction(vaultPda, member.publicKey, LAMPORTS_PER_SOL);
    ...
    const createLookupTableTx = new VersionedTransaction(new TransactionMessage({
    ...
    const batchTransferIxs = Object.values(members).map((member) => createTestTransferInstruction(vaultPda, member.publicKey, LAMPORTS_PER_SOL));
    testTransactionMessages.push({
        message: new TransactionMessage({
            payerKey: vaultPda,
            recentBlockhash: blockhash,
